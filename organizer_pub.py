...
filecount = len([name for name in os.listdir("/dir/path/here/Entropy/pullcapfiles")]) #Put directory location of pullcap directory here
loopcount=0

#Get list of all pullcap files retrieved by the main puller program
pullcaps = os.listdir("/dir/path/here/Entropy/pullcapfiles") #Directory location of pullcap files here

#Write list of pullcap files to text file
d = open("/dir/path/Entropy/pullcaplist.txt",'w') #Location of pullcap list file here
d.writelines(["%s\n" %item for item in pullcaps])
d.close()

#Get UIDs and LATs from all pullcap files
c = open('/dir/path/here/Entropy/uidlist.txt','w+') #Location of UIDlist file here
with open('/dir/path/Entropy/pullcaplist.txt','r') as b: #Location of pullcaplist file here
	for k in b:
		loopcount+=1
		percent = 100 * loopcount / filecount
		sys.stdout.write("\rOrganizing files....%d%% complete." %percent)
		k=k.rstrip()
		a = open('/dir/path/Entropy/pullcapfiles/%s' %k,'r') #Pullcap file directory here
		astr = a.read()
		...
		for x in auid:
			...
			b = open('/dir/path/Entropy/uidfiles/%s.txt' %seluid,"a") #Creates uid.txt file for each unique UID,uidfiles dirpath here
		...
		for z in nuid:
			...
			e = open('/dir/path/Entropy/uidfiles/%s.txt' %neguid,"a") #Creates uid.txt file for each unique UID,uidfiles dirpath here
		...
		for y in unkuid:
			...
			pval = unkmatcher[2]
			vcval = unkmatcher[3]
			g = open('/dir/path/Entropy/uidfiles/%s.txt' %foruid,"a") #Creates uid.txt file for each unique UID, uidfiles dirpath here
